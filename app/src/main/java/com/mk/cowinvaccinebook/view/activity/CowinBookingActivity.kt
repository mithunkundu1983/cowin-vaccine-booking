...
import com.mk.cowinvaccinebook.databinding.ActivityCowinBookingBinding
...
import com.mk.cowinvaccinebook.viewmodel.CowinBookingActivityViewModel
...
class CowinBookingActivity: BaseActivity<CowinBookingActivityViewModel>() {
    ...
    private fun setCurrSessionData(session: Session) {
        ...
        if(session.slots?.count() ?:0  > 1)
            (binding.rgSlots[1] as RadioButton).isChecked = true
        else
